# 念佛計數 PWA（Google 試算表 + Apps Script）

漸進式網頁應用：記錄每日念佛／咒次數，資料寫入**同一份母試算表**中，每個「專案」對應一個**工作表分頁**。可部署於 **GitHub Pages**，後端為 **Google Apps Script Web App**。

## 功能

- **先載入專案**（試算表分頁），再從第 2 列讀出姓名清單供**下拉選擇**
- **新增使用者**：寫入試算表第 2 列下一個空白欄並選取該姓名
- 顯示**本專案累積總計**（第 3 列起所有人次數加總）與**我的累積總計**（目前使用者欄位加總）
- 顯示**今日**次數；`+`／`−` 調整後按「**紀錄**」寫入試算表
- 新分頁請在 Google 試算表手動新增，並維持第 1～2 列標題與 A 欄日期格式
...
- `API_KEY` 用於防止陌生人任意改寫試算表；密鑰會出現在**建置後的 JS** 中（可被進階使用者查看）。若資料極敏感，請改採 Google 登入／更嚴格權限模型。
- 試算表仍應限制「誰能開啟檔案」；Script 以你的帳號執行，需有該試算表編輯權限。
