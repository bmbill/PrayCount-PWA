...
- **新增使用者**：在空白欄寫入姓名並選取
- 顯示**本專案累積總計**與**我的累積總計**；**眾願進度條**（累積 ÷ 目標，目標在 Apps Script 屬性設定）
...
3. **專案設定 → 指令碼屬性** 新增：
   ...
   | `PROJECT_GOAL` | （選填）**眾願進度條**的全域目標數字，預設 `2000000`（200 萬） |
   | `GOALS_JSON` | （選填）依**分頁名稱**覆寫目標，例：`{"度母咒":2000000,"心經":500000}`（JSON 字串，鍵須與試算表分頁名完全一致） |
...
前端選定專案後會以 **`bootstrapSheet`** 單次請求載入使用者名單、總計與今日次數（較省時間）；若 GAS 仍為舊版未含此動作，請貼上最新 [`gas/Code.gs`](gas/Code.gs) 並**重新部署**。
